...
const RecordRow = ({ label, driver, value }) => (
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid var(--line)" }}>
    <div>
      ...
      <div style={{ fontWeight: 600, fontSize: 14, marginTop: 2 }}>{driver}</div>
    </div>
  ...
  return (
    <div className="card" style={{ marginTop: 0 }}>
      ...
      <div style={{
        ...
        color: "var(--mut)", marginTop: 4, marginBottom: 16
      }}>
        {race.Circuit.Location.locality}, {race.Circuit.Location.country}
        {isNew && (
          <span style={{
            marginLeft: 10, fontFamily: "Rajdhani", fontWeight: 700, fontSize: 10,
            letterSpacing: ".1em", color: "var(--gold)",
            background: "rgba(244,196,48,.12)", padding: "2px 7px",
      ...
      {c.blurb && (
        <p style={{
          color: "var(--mut)", fontSize: 13, lineHeight: 1.65,
          margin: "0 0 20px", fontFamily: "Rajdhani", fontWeight: 600
        }}>
          {c.blurb}
        </p>
      )}
      {c.mostWinsDriver?.wins && (
        <div>
          <div style={{
            ...
            letterSpacing: ".18em", color: "var(--mut)",
            textTransform: "uppercase", marginBottom: 4
          }}>Circuit Records</div>
          <RecordRow
            label="Most wins — driver"
            driver={c.mostWinsDriver.name}
            value={c.mostWinsDriver.wins}
          />
          <RecordRow
            label="Most wins — constructor"
            driver={c.mostWinsConstructor.name}
            value={c.mostWinsConstructor.wins}
          />
        </div>
      )}
    </div>
